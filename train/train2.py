...
import time
from models.Text_LSTM import Text_LSTM
from models.Text_RCNN import Text_RCNN
import os
...
def train():
    ...
    #                 encoder_size=opt.encoder_dim, hidden_size=opt.hidden_size, num_classes=opt.num_classes, dropout=0.3,
    #                 pooling='last')
    ...
    for epoch in range(1, opt.max_epoch+1):
        ...
        for data in tqdm(train_loader):
            ...
            if opt.use_gpu:
                txt, _len, label = txt.cuda(), _len.cuda(), label.cuda()
            optimizer.zero_grad()
            out = model(txt, _len)
